...
ofxSimpleHttp::~ofxSimpleHttp()
{
    ...
    if( flushPendingRequestsOnDestruction ) {
        if( queueLenEstimation > 0 && !silent )
            ofLogWarning( "ofxSimpleHttp" ) << "Destructor canceling pending downloads (" << queueLenEstimation << ")";
...
void ofxSimpleHttp::threadedFunction()
{
    ...
    //	if (!timeToStop){ //if we are naturally exiting the thread; if TimeToStop==true it means we are being destructed, and the thread
    // will be joined (so no need to detach!) 		pthread_detach( pthread_self() ); //this is a workaround for this issue
    // https://github.com/openframeworks/openFrameworks/issues/2506
    //	}
    //#endif
}
...
std::string ofxSimpleHttp::drawableString( int urlLen )
{
    ...
    if( isThreadRunning() && n > 0 ) {
        ...
        aux
            = "//// ofxSimpleHttp now fetching //////////////////////////////////\n"
              "//\n"
              "//   URL: "
            + url + "\n" + std::string( r->downloadToDisk ? "//   Save To: " + r->absolutePath + "\n" : "" )
            + std::string( serverSize.length() ? "//   Progress:                 " + spa
                        + std::string( ( r->downloadProgress >= 0.0 ) ? ofToString( 100.0f * r->downloadProgress, 2 ) : "" ) + "%\n" :
                                                 "" )
            + std::string( serverSize.length() ? "//   Server Reported Size:     " + spa + serverSize + "\n" : "" )
...
ofxSimpleHttpResponse ofxSimpleHttp::fetchURLBlocking( std::string url )
{
    ...
    response.checksumType = checksumType;
    bool ok = downloadURL( &response, false /*send res through events*/, true /*beingCalledFromMainThread*/, false /*to disk*/
    );
    return response;
}

void ofxSimpleHttp::fetchURLToDisk(
    std::string url, std::string expectedChecksum, bool notifyOnSuccess, std::string dirWhereToSave, std::string customField )
{
    ...
    std::string savePath
        = dirWhereToSave == "" ? extractFileFromUrl( url ) : ofToDataPath( dirWhereToSave, true ) + "/" + extractFileFromUrl( url );
...
void ofxSimpleHttp::fetchURLToDisk( std::string url, bool notifyOnSuccess, std::string dirWhereToSave, std::string customField )
{
...
ofxSimpleHttpResponse ofxSimpleHttp::fetchURLtoDiskBlocking( std::string url, std::string dirWhereToSave, std::string expectedChecksum )
{
    ...
    std::string savePath
        = dirWhereToSave == "" ? extractFileFromUrl( url ) : ofToDataPath( dirWhereToSave ) + "/" + extractFileFromUrl( url );
    ...
    response.notifyOnSuccess = true;
    response.downloadToDisk = true;
    bool ok = downloadURL( &response, false, /*send result through events*/
...
bool ofxSimpleHttp::downloadURL(
    ofxSimpleHttpResponse *resp, bool sendResultThroughEvents, bool beingCalledFromMainThread, bool saveToDisk )
{
        ...
        if( resp->expectedChecksum.length() ) { // if user provided a checksum
            ...
            if( f.exists() ) {
                ...
                case ofxChecksum::Type::SHA1: {
                    ...
                    ofStringReplace( resp->calculatedChecksum, " ", "" );
                } break;
                ...
                if( fileIsAlreadyHere ) {
                    ...
                    if( !silent )
                        ofLogVerbose( "ofxSimpleHttp" )
                            << "about to download \"" << resp->url << "\" but a file with same name and correct checksum is already here!";
                    if( !silent )
            ...
            if( !onlySkipDownloadIfChecksumMatches ) {
                ...
                if( f.exists() && f.getSize() > 0 ) {
                    ...
                    if( !silent )
                        ofLogVerbose( "ofxSimpleHttp" )
                            << "about to download \"" << resp->url << "\" but a file with same name and (size > 0) is already here!";
                    if( !silent )
        ...
        if( protocol == "file://" ) {
            ...
            if( srcOfFile.exists() ) {
                ...
                if( resp->expectedChecksum.length() > 0 ) {
                    ...
                    if( !resp->checksumOK ) {
                        ...
                        if( !silent )
                            ofLogNotice( "ofxSimpleHttp" )
                                << "Checksum (" + ofxChecksum::toString( resp->checksumType ) + ") was meant to be: \""
                                << resp->expectedChecksum << "\" but is: \"" << resp->calculatedChecksum << "\"";
                    }
                }
            ...
            try {
                ...
                try {
                    ...
                    ofLogWarning( "ofxSimpleHttp" )
                        << "ofxSimpleHttp session send request exception: " << e.what() << " for URL: " << request.url;
                }
                ...
                if( copySize >= 0 ) {
                    ...
                    if( resp->downloadCanceled ) {
                        ...
                        if( resp->timeTakenToDownload > 0.01f ) {
                            ...
                            if( !silent )
                                ofLogNotice( "ofxSimpleHttp" ) << "downloadURL() >> download completed - \"" << resp->url
                                                               << "\" - avg Dl speed: " << bytesToHumanReadable( resp->avgDownloadSpeed, 1 )
                                                               << " - Dl Size: " << bytesToHumanReadable( resp->downloadedBytes, 1 )
...
std::streamsize ofxSimpleHttp::streamCopyWithProgress( std::istream &istr, std::ostream &out, std::streamsize totalBytes,
    std::streamsize &currentBytes, bool &chunkTested, float &progress, float &speed, const bool &cancel )
{
...
std::string ofxSimpleHttpResponse::toString()
{
    ...
    ss << "URL: " << url << endl;
    if( customField.size() )
        ss << "CustomField: " << customField << endl;
    if( ok ) {
        if( fileWasHere ) {
